...
async def test_autenticar_sin_hash_rechaza() -> None:
...
async def test_verificar_token_returns_the_usuario_encoded_in_a_valid_token() -> None:
    provider = JwtAuthProvider(secret_key="test-secret")
    hash_correcto = _ph.hash("cualquier-password")
    token = await provider.autenticar("cliente1", "cualquier-password", password_hash=hash_correcto)
...
async def test_verificar_token_returns_none_when_signed_with_a_different_secret() -> None:
    ...
    hash_correcto = _ph.hash("cualquier-password")
    token = await issuer.autenticar("cliente1", "cualquier-password", password_hash=hash_correcto)
